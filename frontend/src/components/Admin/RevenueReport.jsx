        ...
        <div className="bg-white border border-gray-200 p-6 rounded-3xl space-y-4 shadow-sm lg:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center pt-4">
            ...
            <div className="space-y-3">
              <h5 className="text-gray-500 font-bold text-xs uppercase tracking-wider">Top Phim Có Doanh Thu Cao Nhất</h5>
              <div className="space-y-2.5 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                {movieData.length === 0 ? (
                  <p className="text-xs text-gray-400 italic">Chưa có dữ liệu phim.</p>
                ) : (
                  movieData.map((item, idx) => (
                    <div key={item.name} className="flex justify-between items-center text-xs font-semibold">
                      ...
                      <span className="text-gray-500">{(item.value || 0).toLocaleString('vi-VN')} VNĐ</span>
                    </div>
                  ))
                )}
              </div>
