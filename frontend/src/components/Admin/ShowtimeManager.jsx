...
export const ShowtimeManager = () => {
  ...
  const handleOpenAutoGenerate = () => {
    ...
    setAutoForm({ movieId: movies[0]?._id || '', theaterId: selectedTheater, roomIds: rooms.map((r) => r._id), startDate: today, endDate: nextWeek, timeSlots: [...DEFAULT_TIME_SLOTS], format: '2D' });
  ...
  const handleAutoFormChange = (e) => {
    const { name, value } = e.target;
    setAutoForm((prev) => { const u = { ...prev, [name]: value }; return u; });
  };
                                      ...
                                      return (
                                        <div key={st._id} className="relative group">
                                          <button
                                            ...
                                            title={`Sửa suất chiếu lúc ${startFmt}`}
                                          >
